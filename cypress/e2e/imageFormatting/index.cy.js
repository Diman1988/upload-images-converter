...
describe("Image Convert Test", () => {
  ...
  it("should handle single uploaded image file correctly", () => {
    cy.get("@image1").then((image1) => {
      ...
      cy.window().then((win) => {
        ...
        cy.wrap(imageConvert(fileInput.files)).then((convertedFiles) => {
  ...
  it("should handle single uploaded image file correctly with resolution 1000 * 700", () => {
    cy.get("@image1").then((image1) => {
      ...
      cy.window().then((win) => {
        ...
        cy.wrap(imageConvert(fileInput.files, 1000, 700)).then(
          (convertedFiles) => {
            expect(convertedFiles.length).to.equal(1);
            expect(convertedFiles[0].type).to.equal("image/webp");
  ...
  it("should handle single uploaded image file correctly with resolution 700 * 1000", () => {
    cy.get("@image2").then((image1) => {
      ...
      cy.window().then((win) => {
        ...
        cy.wrap(imageConvert(fileInput.files, 300, 500)).then(
          (convertedFiles) => {
            expect(convertedFiles.length).to.equal(1);
            expect(convertedFiles[0].type).to.equal("image/webp");
